...
abstract class Schema extends BaseObject
{
     ...
     */
    public function getTableSchemas($schema = '', $refresh = false, $relatedTables=[])
    {
       return $this->getSchemaMetadata($schema, 'schema', $refresh, $relatedTables);
    }
     ...
     */
    public function getTableNames($schema = '', $refresh = false, $relatedTables=[])
    {
        if (!isset($this->_tableNames[$schema]) || $refresh) {
            $this->_tableNames[$schema] = $this->findTableNames($schema, $relatedTables);
        }
     ...
     * @since 2.0.13
     */
    protected function getSchemaMetadata($schema, $type, $refresh, $relatedTables=[])
    {
        $metadata = [];
        $methodName = 'getTable' . ucfirst($type);
        foreach ($this->getTableNames($schema, $refresh, $relatedTables) as $name) {
            if ($schema !== '') {
                $name = $schema . '.' . $name;
            }
