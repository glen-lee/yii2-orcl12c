...
SQL;
    ...
    protected function findTableNames($schema = '', $relatedTables=[])
    {
        $rows = (new \yii\db\Query)
            ...
            ->from('ALL_OBJECTS')
            ->where(['OBJECT_NAME' => $relatedTables, 
                    'OWNER' => 'UMN', 
...
SELECT 
    ...
    C.POSITION,
    D.R_CONSTRAINT_NAME,
    CASE WHEN D.CONSTRAINT_TYPE = 'P' THEN null
    WHEN D.CONSTRAINT_TYPE = 'R' THEN
           (SELECT TABLE_NAME FROM U_CONS WHERE OWNER = D.R_OWNER AND CONSTRAINT_NAME = D.R_CONSTRAINT_NAME)
    END AS TABLE_REF,  
    CASE WHEN D.CONSTRAINT_TYPE = 'P' THEN null
    WHEN D.CONSTRAINT_TYPE = 'R' THEN
           (SELECT COLUMN_NAME FROM U_CONS_COL WHERE OWNER = D.R_OWNER AND CONSTRAINT_NAME = D.R_CONSTRAINT_NAME)
    END AS COLUMN_REF, 
    C.TABLE_NAME
FROM U_CONS_COL C
    JOIN U_CONS D ON D.OWNER = C.OWNER AND D.CONSTRAINT_NAME = C.CONSTRAINT_NAME
WHERE
    C.OWNER = :schemaName
